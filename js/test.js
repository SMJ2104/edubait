(async function() {
    ...
    try {
        ...
        // Extract questionMap and create simplified version
        let questionMap = quizData.attempt[quizId].questionMap;
        let simplifiedQuestionMap = {};
        
        if (questionMap) {
            for (let qId in questionMap) {
                simplifiedQuestionMap[qId] = questionMap[qId].question;
            }
            console.log("Extracted Question Map:", simplifiedQuestionMap);
        } else {
            console.log("No questionMap found in quiz data.");
        }
        ...
        // Encode questionMap as Base64 JSON
        let encodedQuestionMap = btoa(JSON.stringify(simplifiedQuestionMap));
